  ...
  <div class="sp_page sp_page_scroll bdBg">
    ...
    <template v-for="msg in {__:recordData}" v-if="msg">
      ...
      <div class="weui-article weui-article-box">
        ...
        <div class="article-desc"><span class="article-author" v-html="msg.senderName">张老师</span> <span
          class="article-time">{{msg.createTime|spoc_date_sim}}</span></div>
        <hr/>
        <section class="article-bd">
          <!--<h2>1.2 小说：以刻画人物形象为中心</h2>-->
          <p v-html="getTitleContent(msg.content)"></p>
        </section>
      </div>
    </template>
...
<script>
  ...
  export default {
    ...
    methods: {
      splitTitle, toWarnPage, getTypeClazz,
      getTitleContent(val){
        const vm = this
        if(val!=null){
